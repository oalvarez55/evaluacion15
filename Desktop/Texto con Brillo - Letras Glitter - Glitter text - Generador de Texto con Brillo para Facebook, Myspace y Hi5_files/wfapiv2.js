if (typeof Wildfire == 'undefined') {
	...
	Wildfire._base = (document.location.protocol == 'https:' ? 'https://cdns.gigya.com/' : 'http://cdn.gigya.com/');
} 
...
Wildfire._createFlashModule = function (moduleType, partner, targetId, width, height, config/*, getParams*/) {
	...
	var sPartner = partner.toString();
	var shouldAddGigyaLink = (!config.hideGigyaLink && height > 120 && sPartner.substr(sPartner.length-2, 1) == '1');
	if (shouldAddGigyaLink) {
		var gigyaLink = module._addGigyaLink();
		module.height -= gigyaLink.offsetHeight + 4;
		config.height = module.height;
		config.hideGigyaLink = true;
	}
	...
	*/
	if (!Wildfire.LinkedLoading || this.modulesArray.length == 1 || this.modulesArray[0].ready) {
		...
		if (shouldAddGigyaLink) module._addGigyaLink();
	} else {
		module.queued = true;
	}
	return module;
...
Wildfire._FlashModule.prototype.init = function (checkPing) {
	...
	var swf = Wildfire._base+'WildFire/swf/wildfire_en.swf';
	if (this.config.lang != null) {
		swf = Wildfire._base + 'WildFire/swf/wildfire_' + this.config.lang + '.swf';
	}
	...
	if (!Wildfire._pixeIframeCreated) {
		html += "<iframe src='"+Wildfire._base+"wildfire/do_not_delete.htm' style='width:0;height:0;visibility:hidden' />";
...
Wildfire._FlashModule.prototype.copyConfig =  function(config) 
{
	...
	if (undef(this.config.progressImageSrc)) this.config.progressImageSrc = Wildfire._base+"WildFire/i/progress_ani.gif";
...
Wildfire._FlashModule.prototype._addGigyaLink = function () {
	var divGigyaSocialLogin = document.createElement('div');
	divGigyaSocialLogin.style.textAlign = 'right';
	divGigyaSocialLogin.innerHTML = '<a href="http://www.gigya.com/social-login/" style="font-size:11px;font-family:arial;color:#005997;text-decoration:none;margin-right:5px">Social login by Gigya</a>';
	this.container.appendChild(divGigyaSocialLogin);
	return divGigyaSocialLogin;
}
...
Wildfire.drawWildfireButton = function(params) {
	...
	if (typeof(params.btnurl)=='undefined') {
		switch(params.conf['module']) {
			case 'bookmarks': params.btnurl = Wildfire._base + 'wildfire/i/bookmark_button.gif'; break;
			case 'email': params['btnurl'] = Wildfire._base + 'wildfire/i/share-button.gif'; break;
			case 'post':
			default: params.btnurl = Wildfire._base + 'wildfire/i/post-to-button.gif';
		}
	}
	switch(params.conf['module']) {
		case 'bookmarks': 
...
Wildfire._showWildfirePopup = function(partner, width, height, configID) {
	...
	if (popDiv==null || popDiv.style.visibility=='hidden') {
		Wildfire._preparePopup(width, height, divID, btnDiv, Wildfire._base + 'WildFire/i/progress_ani.gif');
...
Wildfire._injectCIMP = function() {
	if (document.getElementById('wildfire_cimp') == null && document.body != null) {
		...
		cimp.innerHTML = '<img src="'+Wildfire._base+'wildfire/i/CIMP.gif?CXNID=2000002.0NXC" />';
